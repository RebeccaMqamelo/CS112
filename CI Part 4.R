...
m_2yrs_prop_BA <- Match(Tr=Tr, X=glm_2yrs_prop$fitted, Y=foo$pbs2l, estimand="ATT", M=1, BiasAdjust=TRUE, replace=TRUE)
summary(m_2yrs_prop_BA)

m_2yrs_prop_NBA <- Match(Tr=Tr, X=glm_2yrs_prop$fitted, Y=foo$pbs2l, estimand="ATT", M=1, BiasAdjust=FALSE, replace=TRUE)
summary(m_2yrs_prop_NBA)
...
m_2yrs_mb  <- MatchBalance(Tr ~ wartype + logcost + wardur + factnum + 
                          ...
                          data=foo, match.out = m_2yrs_prop_BA, nboots=500)
# ----------
...
m_5yrs_prop_BA <- Match(Tr=Tr, X=glm_5yrs_prop$fitted, Y=foo$pbs5l, estimand="ATT", M=1, BiasAdjust=TRUE, replace=TRUE)
summary(m_5yrs_prop_BA)

m_5yrs_prop_NBA <- Match(Tr=Tr, X=glm_5yrs_prop$fitted, Y=foo$pbs5l, estimand="ATT", M=1, BiasAdjust=TRUE, replace=TRUE)
summary(m_5yrs_prop_NBA)

m_5yrs_mb  <- MatchBalance(Tr ~ wartype + logcost + wardur + factnum + 
                           factnum2 + trnsfcap + untype4 + treaty + develop + exp + decade + I(logcost*untype4), 
                           data=foo, match.out = m_5yrs_prop_BA, nboots=500)
...
genout <- GenMatch(Tr=Tr, X=X, BalanceMatrix=X_all, estimand="ATT", M=1,
                  pop.size=500, max.generations=50, wait.generations=2, replace = TRUE)

m_2yrs_gen_BA <- Match(Tr=Tr, Weight.matrix=genout, X=X, Y=foo$pbs2l, estimand="ATT", M=1, BiasAdjust=TRUE, replace=TRUE)
summary(m_2yrs_gen_BA)

m_2yrs_gen_NBA <- Match(Tr=Tr, Weight.matrix=genout, X=X, Y=foo$pbs2l, estimand="ATT", M=1, BiasAdjust=FALSE, replace=TRUE)
summary(m_2yrs_gen_NBA)
...
m_2yrs_gen_mb  <- MatchBalance(Tr ~ wartype + logcost + wardur + factnum + 
                              ...
                              data=foo, match.out = m_2yrs_gen_BA, nboots=500)
# ----------
m_5yrs_gen_BA <- Match(Tr=Tr, Weight.matrix=genout, X=X, Y=foo$pbs5l, estimand="ATT", M=1, BiasAdjust=TRUE)
summary(m_5yrs_gen_BA)

m_5yrs_gen_NBA <- Match(Tr=Tr, Weight.matrix=genout, X=X, Y=foo$pbs5l, estimand="ATT", M=1, BiasAdjust=FALSE)
summary(m_5yrs_gen_NBA)

m_5yrs_gen_mb  <- MatchBalance(Tr ~ wartype + logcost + wardur + factnum + 
                                 factnum2 + trnsfcap + treaty + develop + exp + decade + I(logcost*untype4), 
                                 data=foo, match.out = m_5yrs_gen_BA, nboots=500)
